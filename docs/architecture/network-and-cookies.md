...
## Implicit request headers (what the client adds on top of the HEADERS tab)

Dio and dart:io add headers the user never typed. Keep this list in sync with `autoGeneratedHeaders()` (`lib/core/network/auto_generated_headers.dart`), which is what the HEADERS tab shows for them:

- **Content-Type** — Dio's `ImplyContentTypeInterceptor` sends `application/json` for a `String` body with no Content-Type, and `multipart/form-data; boundary=…` for `FormData`. Getman makes the raw case explicit in `BodyTypeUtils.applyContentType` (send path AND code-gen) so the wire, the HEADERS row and generated snippets agree; multipart is still stripped on the send path so Dio owns the boundary.
- **Content-Length** / **Host** — computed by dart:io per send.
- **User-Agent** — dart:io's `Dart/<major.minor> (dart:io)` unless a row sets one (`defaultUserAgent()` rebuilds the string; null on web).
- **Accept-Encoding: gzip** — added by dart:io unless a row sets one (see the content-encoding notes in `network_service.dart` before letting `br`/`zstd` through).
- **Authorization / api-key header** — from the AUTH tab via `resolveAuthApplication` (skip-if-set).
- **Cookie** — from the jar (`cookie_interceptor.dart`); shown in the Cookies manager, not in the HEADERS tab.
