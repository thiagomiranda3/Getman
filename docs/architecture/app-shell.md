...
- **`RequestView`** hosts `SaveRequestIntent` and `BeautifyJsonIntent`. `MainScreen` ALSO hosts a `SaveRequestIntent` fallback (K2): with focus outside the RequestView subtree (HISTORY search box, collections filter field) the chord used to be dead. Actions dispatch picks the nearest enclosing handler, so RequestView's richer action (it can open the save-to-collection dialog) still wins while focus is inside the tab; the shell fallback mirrors `planSaveAllTabs`' single-tab semantics (dirty gate via `TabDirtyChecker`, `UpdateNodeRequest` for a dirty linked tab, an informational snackbar for a dirty unlinked one — the name-prompt dialog is RequestView's flow). `BeautifyJsonIntent` deliberately has no shell entry: it formats RequestView's body-editor controller, which the shell doesn't own.

Put intents where `context.read<TabsBloc>()` is reachable — and dialog-opening ones *below* `MaterialApp`. **Index-reading MainScreen actions must live-read `context.read<TabsBloc>().state` inside `onInvoke`** (K4) — the builder snapshot is one frame stale, so e.g. a Ctrl+Tab + Cmd/Ctrl+W chord inside one frame would close the tab the user just left.
