...
- **Requests:** each call is a discrete JSON-RPC POST (`initialize` / `tools/list` / `tools/call`) carrying the session id + protocol-version headers. The response may arrive as a plain `application/json` body **or** a `text/event-stream` body, parsed INCREMENTALLY via `SseParser` — the call completes the moment an event carries the matching `id` AND a `result`/`error` (id compared via `toString()`, batch arrays unwrapped, server-initiated requests/notifications skipped), then the still-open stream is cancelled, so a server that never closes its SSE stream can't hang the call.
