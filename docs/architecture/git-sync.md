...
`GitConflictService` classifies each conflicted path over the paused rebase: `.req.json` and `.folder.json` files are parsed at merge stages 1/2/3 (base/incoming/yours) and field-level 3-way merged via `ThreeWayMerge`. Delete/modify (one side's stage absent), add/add (no base), and unparseable stages (→ structural) are distinguished. The user's picks are applied on top of the merged skeleton, serialized, written to the working tree, and staged; then `rebase --continue` (identity threaded) advances to the next batch or finishes. `rebase --continue` exits 0 even when the finished rebase's **autostash re-apply conflicted** — `continueRebase` checks `conflictedPaths` after a finished rebase, restores the clean tree (`resetHard`; the edits stay in `stash@{0}`) and returns `RebaseStep.doneEditsStashed`, which the dialog surfaces with the same "kept in the git stash" message as `PullOutcome.cleanEditsStashed` — mirroring `pull()`'s handling of the identical git behavior.

### Reload after a tree-changing op

`BranchSyncListener` watches `GitSyncBloc` and, on a reload/conflict token bump, re-reads the forest from disk and dispatches `ReplaceCollections` — with mirroring suspended (to avoid a reload → mirror → reload loop) — carrying along any open tabs whose linked request is untouched so they show the new version. A FAILED reload (e.g. one malformed `.req.json` in the pulled tree) is never silent: `WorkspaceSyncService.read` sticky-blocks mirroring for that root (`isReloadBlocked`) until a later read succeeds — Hive no longer matches disk, and a resumed mirror would overwrite the pulled files with the stale forest (deleting teammates' new requests) — and the listener surfaces a snackbar pointing at RELOAD FROM DISK.

## Flows

1. **Review & commit:** BranchChip / review button → `ReviewChangesDialog` (`ReviewBloc` → `ReviewService.review`, which flushes the mirror then diffs `git status` semantically) → stage/unstage (a select-all is one git call) → commit (identity threaded). git's index is the source of truth, so each mutation re-runs the review. `commit` **re-stages** entries edited after staging (`MM`/`MD`) first — the dialog previews the WORKTREE content, so committing the older index blob would record something other than what was reviewed. Dirtiness for branch switching (`GitBranchService.isDirty`) counts only workspace paths (`workspace_paths.dart` — the exact file set Review can commit), so `.DS_Store` and other junk can't wedge a switch behind an "uncommitted changes" prompt an empty Review can never clear. `isRebaseInProgress` handles linked `git worktree` checkouts, where `--git-path` answers with an ABSOLUTE path.
