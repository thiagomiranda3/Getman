...
void main() {
  group('UrlFocusRegistry', () {
    testWidgets('focus() focuses the node registered for the given tab', (tester) async {
      ...
      registry.register('a', nodeA);
      registry.register('b', nodeB);
    ...
    test('unregister only drops the node when it is still the registered one', () {
      final registry = UrlFocusRegistry();
      ...
      registry.register('a', oldNode);
      // A fresh UrlBar for the same id registered before the old one's dispose.
      registry.register('a', newNode);
      // The old one's dispose must not clobber the new registration.
      registry.unregister('a', oldNode);

      expect(() => registry.focus('a'), returnsNormally);
      // Re-registering newNode is a no-op overwrite; unregister with it clears.
