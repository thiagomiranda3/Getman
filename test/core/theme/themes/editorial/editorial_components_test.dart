...
void main() {
  ...
  setUpAll(() {
    ...
    light = editorialTheme(Brightness.light);
  });
  ...
  testWidgets('methodBadge → TypographicTag', (tester) async {
    await _pump(
      tester,
      light,
    ...
    expect(find.byType(TypographicTag), findsOneWidget);
  });

  testWidgets('statusBadge → TypographicTag', (tester) async {
    await _pump(
      tester,
      light,
  ...
  testWidgets('surface (no title) fills + no overflow', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(ArticlePanel), findsOneWidget);
    expect(find.text('BODY'), findsOneWidget);
  });

  testWidgets('surface (title) → ArticlePanel shows section heading', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets('metric is inline-safe in a tight Wrap', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(FootnoteMetric), findsNWidgets(2));
  });

  testWidgets('toggle → OutlinedSwitch (tap flips)', (tester) async {
    var v = false;
    await _pump(
      tester,
      light,
  ...
  testWidgets('logView sizes to bounded height (no overflow)', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(DispatchLog), findsOneWidget);
  });
  ...
  testWidgets('statusBanner → EditorialNoteBar', (tester) async {
    await _pump(
      tester,
      light,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(EditorialNoteBar), findsOneWidget);
  });

  testWidgets('pendingIndicator → GalleyProof (static, settles at once)', (
    tester,
  ) async {
    await _pump(tester, light, (c) => c.appComponents.pendingIndicator(c));
    expect(find.byType(GalleyProof), findsOneWidget);
    // GalleyProof is purely static — no controller schedules frames, so
    // pumpAndSettle returns immediately even at full effects.
    await tester.pumpAndSettle();
    expect(tester.takeException(), isNull);
  });
  ...
  testWidgets(
    'ResponseSection metadata row + body render under editorialTheme '
    'without overflow',
    (tester) async {
      const tabId = 'editorial_resp';
  ...
  testWidgets('RealtimePanel renders under editorialTheme without overflow', (
    tester,
  ) async {
    const tabId = 'editorial_rt';
    ...
    expect(tester.takeException(), isNull);
    // Editorial-specific slots must be used (not the default fallbacks).
    expect(find.byType(DispatchLog), findsOneWidget);
    expect(find.byType(EditorialNoteBar), findsOneWidget);
  });
}
