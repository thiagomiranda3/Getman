...
import 'package:getman/core/theme/themes/rpg/rpg_components.dart';
...
void main() {
  ...
  testWidgets('methodBadge → RunePlateBadge', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(find.byType(RunePlateBadge), findsOneWidget);
  });
  ...
  testWidgets('statusBadge → GemBadge', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(GemBadge), findsWidgets);
  });
  ...
  testWidgets('surface (no title) fills + no overflow', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(RunicPanel), findsOneWidget);
    expect(find.text('BODY'), findsOneWidget);
  });

  testWidgets('surface (title) shows header banner', (tester) async {
    await _pump(
      tester,
      dark,
  ...
  testWidgets('metric is inline-safe in a tight Wrap', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(RunestoneChip), findsNWidgets(2));
  });

  testWidgets('toggle → EnchantedLever (tap flips)', (tester) async {
    var v = false;
    await _pump(
      tester,
      dark,
  ...
  testWidgets('logView sizes to bounded height (no overflow)', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(GrimoireLog), findsOneWidget);
  });

  testWidgets('dataRow → QuestLedgerRow', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(QuestLedgerRow), findsOneWidget);
  });

  testWidgets('statusBanner → HeraldicBanner', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(HeraldicBanner), findsOneWidget);
  });

  testWidgets(
    'pendingIndicator → SummoningRing animates then disposes cleanly',
    (tester) async {
      await _pump(tester, dark, (c) => c.appComponents.pendingIndicator(c));
      expect(find.byType(SummoningRing), findsOneWidget);
  ...
  testWidgets('reduceEffects: pending + toggle render static (no ticker)', (
    ...
    final reduced = rpgTheme(Brightness.dark, reduceEffects: true);
    await _pump(
      tester,
      reduced,
      (c) => Column(
  ...
  testWidgets(
    'ResponseSection metadata row + body render under rpgTheme '
    'without overflow',
    (tester) async {
      const tabId = 'rpg_resp';
  ...
  testWidgets('RealtimePanel renders under rpgTheme without overflow', (
    tester,
  ) async {
    const tabId = 'rpg_rt';
    ...
    expect(tester.takeException(), isNull);
    // RPG-specific slots must be used (not the Brutalist fallbacks).
    expect(find.byType(GrimoireLog), findsOneWidget);
    expect(find.byType(HeraldicBanner), findsOneWidget);
  });
}
