...
void main() {
  ...
  setUpAll(() {
    ...
    dark = glassTheme(Brightness.dark);
  });
  ...
  testWidgets('methodBadge → GlassLozenge', (tester) async {
    await _pump(
      tester,
      dark,
  ...
  testWidgets('statusBadge → GlassLozenge', (tester) async {
    await _pump(
      tester,
      dark,
  ...
  testWidgets('surface (title) → FrostedTile shows header chip', (
    tester,
  ) async {
    await _pump(
      tester,
  ...
  testWidgets('metric is inline-safe in a tight Wrap', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(FrostedLozengeMetric), findsNWidgets(2));
  });

  testWidgets('toggle → LiquidSwitch (tap flips)', (tester) async {
    var v = false;
    await _pump(
      tester,
      dark,
  ...
  testWidgets('logView sizes to bounded height (no overflow)', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(BlurredTerminalLog), findsOneWidget);
  });

  testWidgets('dataRow → GlassDataRow', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(GlassDataRow), findsOneWidget);
  });

  testWidgets('statusBanner → FrostedCapsuleBanner', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(FrostedCapsuleBanner), findsOneWidget);
  });

  testWidgets(
    'pendingIndicator → FrostedRipple animates then disposes cleanly',
    (tester) async {
      await _pump(tester, dark, (c) => c.appComponents.pendingIndicator(c));
      expect(find.byType(FrostedRipple), findsOneWidget);
  ...
  testWidgets('reduceEffects: pending + toggle render static (no ticker)', (
    tester,
  ) async {
    final reduced = glassTheme(Brightness.dark, reduceEffects: true);
    await _pump(
      tester,
      reduced,
      (c) => Column(
    ...
    expect(tester.takeException(), isNull);
    // Static pending indicator schedules no frames: pumpAndSettle returns at
    // once (no controller exists in reduced mode).
    await tester.pumpAndSettle();
    expect(find.byType(FrostedRipple), findsOneWidget);
  });
  ...
  testWidgets(
    'ResponseSection metadata row + body render under glassTheme '
    'without overflow',
    (tester) async {
      const tabId = 'glass_resp';
  ...
  testWidgets('RealtimePanel renders under glassTheme without overflow', (
    tester,
  ) async {
    const tabId = 'glass_rt';
    ...
    expect(tester.takeException(), isNull);
    // Glass-specific slots must be used (not the Brutalist/default fallbacks).
    expect(find.byType(BlurredTerminalLog), findsOneWidget);
    expect(find.byType(FrostedCapsuleBanner), findsOneWidget);
  });
}
