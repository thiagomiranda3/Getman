// Dracula component-slot smoke + integration tests.
//
// Pattern: pump each slot inside a bounded SizedBox; check no exception and
// the expected widget type is found. Mirrors the glass/editorial template.
...
void main() {
  ...
  setUpAll(() {
    ...
    dark = draculaTheme(Brightness.dark);
  });
  ...
  testWidgets('methodBadge → NeonCapsule', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(find.byType(NeonCapsule), findsOneWidget);
  });

  testWidgets('statusBadge → NeonCapsule', (tester) async {
    await _pump(
      tester,
      dark,
  ...
  testWidgets('surface (title) → ConsolePanel shows comment header', (
    tester,
  ) async {
    await _pump(
      tester,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(ConsolePanel), findsOneWidget);
    // The comment header should render "// PANEL" in the title area.
  ...
  testWidgets('metric is inline-safe in a tight Wrap', (tester) async {
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(TerminalMetric), findsNWidgets(2));
  });

  testWidgets('toggle → ConsoleToggle (tap flips)', (tester) async {
    var v = false;
    await _pump(
      tester,
      dark,
  ...
  testWidgets('dataRow → ConsoleKvRow', (tester) async {
    await _pump(
      tester,
      dark,
      (c) => c.appComponents.dataRow(
        c,
        label: 'content-type',
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(ConsoleKvRow), findsOneWidget);
  });

  testWidgets('statusBanner → ConsoleStatusLine', (tester) async {
    await _pump(
      tester,
      dark,
    ...
    expect(tester.takeException(), isNull);
    expect(find.byType(ConsoleStatusLine), findsOneWidget);
  });

  testWidgets(
    'pendingIndicator → BlinkingCursor animates then disposes cleanly',
    (tester) async {
      await _pump(tester, dark, (c) => c.appComponents.pendingIndicator(c));
      expect(find.byType(BlinkingCursor), findsOneWidget);
      // Advance time: cursor blink period ≥ 666ms → at most 1.5 Hz.
  ...
  testWidgets('reduceEffects: BlinkingCursor renders steady (no controller)', (
    tester,
  ) async {
    final reduced = draculaTheme(Brightness.dark, reduceEffects: true);
    await _pump(
      tester,
      reduced,
      (c) => SizedBox(
        height: 60,
        child: c.appComponents.pendingIndicator(c),
      ),
    );
    expect(tester.takeException(), isNull);
    // Static cursor schedules no frames: pumpAndSettle returns at once.
    await tester.pumpAndSettle();
    expect(find.byType(BlinkingCursor), findsOneWidget);
  });
  ...
  testWidgets(
    'ResponseSection metadata row + body render under draculaTheme '
    'without overflow',
    (tester) async {
      const tabId = 'dracula_resp';
  ...
  testWidgets('RealtimePanel renders under draculaTheme without overflow', (
    tester,
  ) async {
    const tabId = 'dracula_rt';
    ...
    expect(tester.takeException(), isNull);
    // Dracula-specific slots must be used (not the default fallbacks).
    expect(find.byType(DevConsoleLog), findsOneWidget);
    expect(find.byType(ConsoleStatusLine), findsOneWidget);
  });
}
