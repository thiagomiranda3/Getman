...
void main() {
  group('slugFilename', () {
    test('lowercases and replaces non-alphanumeric runs with single underscores', () {
  ...
  group('importSummaryMessage', () {
    test('reports a plain success count when nothing failed', () {
      expect(
        importSummaryMessage(importedCount: 2, failures: const [], noun: 'collection'),
    ...
    test('reports failure-only imports', () {
      expect(
        importSummaryMessage(importedCount: 0, failures: const ['a.json: bad'], noun: 'collection'),
    ...
    test('reports partial success with skipped files', () {
      expect(
        importSummaryMessage(
            importedCount: 1, failures: const ['a.json: bad', 'b.json: worse'], noun: 'environment'),
        'Imported 1 environment(s). Skipped: a.json: bad; b.json: worse',
      );
    });

    test('returns null when there is nothing to report', () {
      expect(importSummaryMessage(importedCount: 0, failures: const [], noun: 'collection'), isNull);
    });
  });
}
