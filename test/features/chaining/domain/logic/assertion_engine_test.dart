...
void main() {
  ...
  test('status code equals', () {
    expect(run(const Assertion(id: '1', expected: '201')).passed, isTrue);
    expect(run(const Assertion(id: '1', expected: '200')).passed, isFalse);
  });
    ...
    expect(
      run(
        const Assertion(
          ...
          comparator: AssertionComparator.lessThan,
          expected: '500',
        ),
      ).passed,
      isTrue,
    ...
    expect(
      run(
        const Assertion(
          id: '1',
          target: AssertionTarget.responseTime,
          comparator: AssertionComparator.greaterThan,
          expected: '500',
        ),
      ).passed,
      isFalse,
  ...
  test('notEquals requires the target to be present', () {
    ...
    expect(
      run(
        const Assertion(
          id: '1',
          target: AssertionTarget.bodyJsonPath,
          comparator: AssertionComparator.notEquals,
          path: 'token',
          expected: 'xyz',
        ),
      ).passed,
    ...
    // Absent -> must NOT pass merely because the "(not found)" sentinel
    // differs.
    expect(
      run(
        const Assertion(
  ...
  test('disabled assertions are skipped', () {
    final r = AssertionEngine.run(const [
      Assertion(id: '1', enabled: false, expected: '201'),
    ], response);
    expect(r, isEmpty);
  });
}
