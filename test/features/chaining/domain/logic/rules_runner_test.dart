...
void main() {
  const extractionRules = [
    ...
    ExtractionRule(id: '2', kind: ExtractionKind.jsonPath, expression: 'user.id', targetVariable: 'uid'),
    ExtractionRule(id: '3', kind: ExtractionKind.header, expression: 'x-id', targetVariable: 'rid'),
  ];
  const assertions = [
    Assertion(id: '1', target: AssertionTarget.statusCode, expected: '200'),
    Assertion(id: '2', target: AssertionTarget.bodyJsonPath, path: 'token', expected: 'abc123'),
    Assertion(id: '3', target: AssertionTarget.bodyJsonPath, path: 'user.id', comparator: AssertionComparator.exists),
  ];

  RulesRunOutput runFor(HttpResponseEntity response) => runRules(RulesRunInput(
  ...
  test('runRules matches the per-engine path for a valid JSON body', () {
    ...
    expect(out.extraction.map((e) => '${e.variable}=${e.value}'),
        extraction.map((e) => '${e.variable}=${e.value}'));
    expect(out.assertions.map((a) => '${a.label}:${a.passed}'),
  ...
  test('non-JSON body: jsonPath rules miss, header/status still work', () {
    ...
    expect(out.assertions.map((a) => a.passed),
        AssertionEngine.run(assertions, response).map((a) => a.passed));
  });

  test('empty body behaves like a parse miss', () {
    const response = HttpResponseEntity(statusCode: 204, body: '', headers: {}, durationMs: 5);
    final out = runFor(response);
  ...
  test('decodes once for many jsonPath rules (results unaffected by rule count)', () {
    final manyRules = List.generate(
      30,
      (i) => ExtractionRule(
          id: '$i', kind: ExtractionKind.jsonPath, expression: 'token', targetVariable: 'v$i'),
    );
    const response = HttpResponseEntity(
      statusCode: 200,
      body: '{"token":"abc123"}',
    ...
    final out = runRules(
        RulesRunInput(extractionRules: manyRules, assertions: const [], response: response));
