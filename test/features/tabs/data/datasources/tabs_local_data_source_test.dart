...
void main() {
  ...
  group('legacy migration', () {
    test('re-keys an int-keyed box by tabId preserving insertion order', () async {
  ...
  group('order reconciliation', () {
    test('drops order entries whose tab no longer exists and appends unknown tabs', () async {
      await tabsBox.putAll({'a': model('a'), 'b': model('b'), 'c': model('c')});
      // 'ghost' has no box entry; 'b' is in the box but missing from the order.
      await metaBox.put(TabsLocalDataSourceImpl.orderKey, ['c', 'ghost', 'a']);
  ...
  group('incremental writes', () {
    ...
    test('saveTabs rewrites the whole box keyed by tabId plus the order', () async {
