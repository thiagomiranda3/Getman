...
- **realtime** — WebSocket (`web_socket_channel`) + SSE (`SseParser` over a `dio` response
  ...
  `WebSocketChannel.connect`) so WS teardown is unit-testable with a fake channel. The SSE Dio is
  built via `RealtimeService.buildSseDio(NetworkConfig, [cookieInterceptor])` — same
  verify-SSL/proxy/mTLS adapter and cookie jar as the main client — and
  `NetworkSettingsListener` calls `RealtimeService.applyConfig` alongside
  `NetworkService.applyConfig` on settings changes. SSE surfaces non-2xx connects as an
  `HTTP <code>` error frame (it never silently streams an error body); binary WS frames log as a
  `[binary frame · N bytes]` placeholder. (WS proxy/mTLS is a known follow-up — plain
  `WebSocketChannel.connect` doesn't take the custom adapter.)
...
`SettingsModel` also carries network/redirect/mTLS fields: `int maxRedirects` at `HiveField(18)` (default `5`, stored on `BaseOptions.maxRedirects` but enforced by `NetworkService`'s **manual redirect loop** — each hop is sent with `followRedirects: false` so the cookie interceptor runs per hop; a login 302's `Set-Cookie` is captured and re-matched on the next hop, 303 / POST-301/302 become bodyless GETs, 307/308 keep method+body, and the `Authorization` header is stripped when a redirect crosses hosts), and the client-certificate trio `String? clientCertPath` / `clientKeyPath` / `clientCertPassphrase` at `HiveField(19/20/21)` (nullable; mTLS). `applyConfig` (both `NetworkService` and `RealtimeService`) rebuilds the HTTP adapter only when an adapter-relevant field changed (`NetworkConfig.sameAdapterConfig`) and closes the replaced adapter — timeout/redirect edits mutate `BaseOptions` in place. Cert config is plain-string data on `NetworkConfig`/`SettingsEntity` (never a `dart:io` `SecurityContext`, which is built only inside `dio_adapter_config_io.dart`, guarded with a try/catch fallback; the web stub ignores it). All reach the live Dio via `NetworkSettingsListener.listenWhen`.
...
`SettingsModel` also carries auto-update fields: `bool checkForUpdatesOnStartup` at `HiveField(25)` (default `true`; toggled via `UpdateCheckForUpdatesOnStartup`) and `String? skippedUpdateVersion` at `HiveField(26)` (nullable; set via `SetSkippedUpdateVersion` when the user picks "Skip this version"; a still-newer release prompts again because `shouldPromptForUpdate` only suppresses the exact skipped version — the stored value is never cleared). The git commit identity lives at `HiveField(28)` (`String? gitUserName`) and `HiveField(29)` (`String? gitUserEmail`) — used inline (`git -c user.name=…`) by the git-sync feature, never written to the user's global git config. **Retired fields — do not reuse these indices:** `HiveField(22)` (`reduceVisualEffects`, removed) and `HiveField(27)` (`enableThemeSounds`, removed). (Next free: 30.)
...
6. `Shortcuts` + `Actions` wire global keyboard intents: **Ctrl/Cmd+N** new tab, **+W** close tab, **+S** save, **+Enter** send, **+B** beautify JSON, **+K** command palette, **+L** focus URL, **Ctrl+Tab / Ctrl+Shift+Tab** next/prev tab, **Cmd/Ctrl+1–9** jump to tab, **+Shift+N** new panel, **+Shift+] / +Shift+[** next/prev panel, **+Shift+1–9** jump to panel. The shortcut map is the computed `appShortcuts` in `main.dart` (a built map, not a `const` literal — the digit→`JumpToTabIntent` bindings are generated in a loop; `@visibleForTesting`). Only the `Shortcuts` map lives at the root; **every** `Action` lives in `MainScreen` (or deeper). A root `Actions` above `MaterialApp` is reachable from focused widgets *inside every modal dialog* (dialogs push onto the same root Navigator), which is how Cmd/Ctrl+N used to stack invisible tabs behind the settings dialog / command palette — `MainScreen`'s `Actions` is a sibling of dialog routes, so its shortcuts are correctly dead while a dialog is up. Tab-switch + focus-URL `Action`s also need `activeIndex`/`tabs`/`UrlFocusRegistry`, and the dialog-opening `CommandPaletteIntent` (+K) / `SwitchEnvironmentIntent` (+E) need a context below `MaterialApp` (`MaterialLocalizations`) and the router's `Navigator`.
...
- **`HttpRequestConfig.==` / `hashCode` deliberately exclude `id`** so history dedup works on request signature. The signature spans `method + url + body` plus `bodyType`/`graphqlVariables`/`bodyFilePath`/`formFields` (added so distinct GraphQL/binary/multipart sends don't dedupe each other away). Do not re-include `id` without a discussion.
...
### Workflow
- **Verify before claiming done**: a feature is NOT done until the full static-analysis stack is clean **and** tests are green. Run, and confirm zero issues from, **all** of: `fvm flutter analyze` (very_good_analysis), `fvm dart run custom_lint` (Getman architecture rules), and `fvm dart run bloc_tools:bloc lint lib` (bloc_lint), and the getman_lints fixtures self-test (`cd tools/getman_lints/example && fvm dart run custom_lint`) — plus `fvm dart format` cleanliness and `fvm flutter test`. These are separate passes; never assume a clean `flutter analyze` covers custom_lint or bloc_lint. The `.githooks/pre-commit` hook enforces the analysis + format gate on every commit (`git config core.hooksPath .githooks` to enable per clone; bypass only via `--no-verify`).
- **New custom_lint rules go in `tools/getman_lints/`**: it is its own package (`custom_lint_builder` plugin) excluded from the app analyzer. Every new rule **must ship a `// expect_lint:` fixture** in `tools/getman_lints/example/lib/` — the fixtures self-test is part of the pre-commit hook and CI gate so rule regressions are caught immediately. Run `( cd tools/getman_lints/example && fvm dart run custom_lint )` to verify fixtures locally. Suppress a justified exception with a per-line `// ignore: <rule>` + a one-line reason (note: `equatable_props_complete` is anchored to the class name, so the `// ignore:` must be directly above the class declaration, not above a field).
  ...
  - `bloc_depends_on_abstractions` — BLoC files (matched by `*_bloc.dart` / `*_cubit.dart`) must not import concrete data-layer packages (`package:hive_ce/`, `package:dio/`, or project `*/data/*.dart` paths). BLoCs depend on abstract Repository types only.
  ...
  - `equatable_props_complete` — classes that **directly** `extends Equatable` or `with EquatableMixin` must include all declared instance fields in `props`; missing fields cause silent equality bugs. (Indirect subclasses — e.g. `SendRequest extends TabsEvent extends Equatable` — are not checked; only the class that directly extends/mixins Equatable is.)
