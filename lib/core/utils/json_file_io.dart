...
//
// Gotchas: slugFilename / saveJsonFileWithFeedback / importJsonFilesWithFeedback
// are the canonical entry points — don't reimplement picker/snackbar logic
// per feature. saveJsonFileWithFeedback (and the underlying
// saveTextFileWithFeedback) take an `allowedExtensions` param (default
// `['json']`); the response body's Save action passes `['json','txt']`. On
// web the picker's `bytes:` already triggers the download, so the redundant
// `File(...).writeAsString` is skipped behind a `kIsWeb` guard.
