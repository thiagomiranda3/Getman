...
A bugfix release for request headers and code generation. The `Content-Type`
header now always shows up — and follows the body type — in the HEADERS tab,
in imported requests, and in every generated snippet, so a cURL command
generated by Getman imports into Bruno or Postman with the right body mode.
The HEADERS tab also lists the headers the HTTP client adds on its own.
